...
public class SyncAllRfiTask extends JobService {
    private List<Rfi> listToSync;
    ...
    public boolean onStartJob(JobParameters params) {
        if (!isWorking) {
            ...
            listToSync = new Gson().fromJson(jobString, new TypeToken<List<Rfi>>() {
            }.getType());
            ...
            if (syncedList.contains(listToSync.get(0).getId())) {
                ...
                new AsyncTask<Void, Void, Long>() {
                    ...
                    protected Long doInBackground(Void... voids) {
                        Rfi rfi = listToSync.get(0);
                        downloadsAllFiles(rfi);
                        return BuilderStormApplication.mLocalDatabase.insertRFI(
                                BuilderStormApplication.mPrefs.getSiteId(),
                                BuilderStormApplication.mPrefs.getUserId(),
                                rfi.getProjectId(), rfi.getId(),
    ...
    private void downloadsAllFiles(Rfi rfi) {
        if (null != rfi.getAttachments()) {
    ...
    private void updateSyncedList() {
        syncedList.clear();
        syncedList.addAll(BuilderStormApplication.mLocalDatabase.getRfiIdList(BuilderStormApplication.mPrefs.getSiteId(),
