...
public class SyncAllCompanyDocsTask extends JobService {
    private List<CompanyDocument> listToSync;
    ...
    public boolean onStartJob(JobParameters params) {
        if (!isWorking) {
            ...
            listToSync = new Gson().fromJson(jobString, new TypeToken<List<CompanyDocument>>() {
            }.getType());
            ...
            if (syncedList.contains(listToSync.get(0).getId())) {
                ...
                new AsyncTask<Void, Void, Long>() {
                    ...
                    protected Long doInBackground(Void... voids) {
                        CompanyDocument doc = listToSync.get(0);
                        doc.setDocSynced(true);
                        downloadsAllFiles(doc);
                        return BuilderStormApplication.mLocalDatabase.insertCompanyDocs(doc.getId(),
                                BuilderStormApplication.mPrefs.getSiteId(),
                                BuilderStormApplication.mPrefs.getUserId(),
                                doc.getCategory(), new Gson().toJson(doc));
                    }
    ...
    private void downloadsAllFiles(CompanyDocument doc) {
        if (!doc.getFilename().isEmpty()) {
            CommonMethods.download(getDownloadManager(), doc.getFilelocation(), doc.getFilename(),
                    getApplication(), "");
        }
    ...
    private void updateSyncedList() {
        syncedList.clear();
        syncedList.addAll(BuilderStormApplication.mLocalDatabase.getSyncedCompanyDocs(
                BuilderStormApplication.mPrefs.getSiteId(),
                BuilderStormApplication.mPrefs.getUserId()));
